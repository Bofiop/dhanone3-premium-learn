...
import portraitAsset from "@/assets/dhanone3-portrait.png.asset.json";
import tradeGoldPlan from "@/assets/trade-chart-gold-plan.jpg.asset.json";
import tradeGoldSetup from "@/assets/trade-chart-gold-setup.jpg.asset.json";
import tradeNifty from "@/assets/trade-chart-nifty.jpg.asset.json";
import tradeReversal from "@/assets/trade-chart-reversal.jpg.asset.json";
import tradeLiquidity from "@/assets/trade-chart-liquidity.jpg.asset.json";
import reviewPraveen from "@/assets/review-praveen.jpg.asset.json";
import reviewTakdir from "@/assets/review-takdir.jpg.asset.json";
import reviewSarthak from "@/assets/review-sarthak.jpg.asset.json";
import reviewKhemraj from "@/assets/review-khemraj.jpg.asset.json";
...
const tradeImages = [tradeGoldPlan, tradeGoldSetup, tradeNifty, tradeReversal, tradeLiquidity];
const tradeLabels = ["MARKET STRUCTURE", "LIQUIDITY MAP", "PRICE ACTION", "TRADE JOURNAL", "TRADING SETUP"];
const reviewImages = [reviewPraveen, reviewTakdir, reviewSarthak, reviewKhemraj];
  ...
  return <main className="bg-background text-foreground">
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:px-8">
        <div className="reveal-item">
          <div className="glass-panel relative flex aspect-[4/5] items-center justify-center overflow-hidden">
            <div className="absolute inset-0 grid-field opacity-50"/><div className="absolute inset-x-[12%] bottom-0 top-[10%] rounded-full bg-primary/10 blur-3xl"/><img src={portraitAsset.url} alt="DhanOne3 trading educator and mentor" className="relative z-10 size-full object-contain object-bottom drop-shadow-2xl"/>
          </div>
        </div>
    ...
    <section id="proofs" className="section-pad bg-secondary/35">
      <div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="reveal-item max-w-3xl"><Eyebrow>SHARED TRADE EXAMPLES</Eyebrow><h2 className="mt-5 font-display text-4xl font-bold leading-[1.02] sm:text-6xl">REAL TRADES.<br/>REAL CHARTS.<br/><span className="text-muted-foreground">REAL JOURNEY.</span></h2><p className="mt-6 text-muted-foreground">A selection of shared trade examples, market setups and chart analysis.</p></div>
        <div className="mt-12 grid gap-4 md:grid-cols-12">{tradeImages.map((asset,i)=><button key={asset.url} onClick={()=>setLightbox(i)} className={`group reveal-item relative min-h-64 overflow-hidden border border-border bg-card text-left ${i===0||i===3 ? "md:col-span-7" : "md:col-span-5"} ${i===4 ? "md:col-span-12" : ""}`}><img src={asset.url} alt={`${tradeLabels[i]} chart shared by DhanOne3`} loading="lazy" className="absolute inset-0 size-full object-cover transition-transform duration-500 group-hover:scale-[1.03]"/><span className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-background to-transparent"/><span className="absolute bottom-5 left-5 text-xs font-semibold text-foreground">{tradeLabels[i]}</span><span className="absolute right-5 top-5 flex size-9 items-center justify-center border border-border bg-background/70 opacity-0 backdrop-blur transition-opacity group-hover:opacity-100"><ExternalLink className="size-4" /></span></button>)}</div>
    ...
    {lightbox !== null && <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 p-4 backdrop-blur" role="dialog" aria-modal="true" aria-label="Trade chart preview"><Button variant="outline" size="icon" className="absolute right-5 top-5" onClick={()=>setLightbox(null)}><X/></Button><div className="relative flex max-h-[84vh] w-full max-w-6xl items-center justify-center overflow-hidden border border-border bg-card"><img src={tradeImages[lightbox]?.url} alt={`${tradeLabels[lightbox]} chart enlarged`} className="max-h-[84vh] w-full object-contain"/></div></div>}
  </main>;
}
