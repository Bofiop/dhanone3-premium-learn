  ...
  return <main className="bg-background text-foreground">
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:px-8">
        <div className="reveal-item">
          <div className="glass-panel relative flex aspect-[4/5] items-center justify-center overflow-hidden">
            <div className="absolute inset-0 grid-field opacity-50"/><div className="relative text-center"><div className="mx-auto mb-5 flex size-20 items-center justify-center rounded-full border border-primary/30 bg-primary/10"><Sparkles className="size-8 text-primary" /></div><p className="font-display text-lg font-semibold">PORTRAIT COMING SOON</p><p className="mt-2 text-sm text-muted-foreground">Professional portrait reserved here</p></div>
          </div>
        </div>
    ...
    <section id="proofs" className="section-pad bg-secondary/35">
      <div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="reveal-item max-w-3xl"><Eyebrow>SHARED TRADE EXAMPLES</Eyebrow><h2 className="mt-5 font-display text-4xl font-bold leading-[1.02] sm:text-6xl">REAL TRADES.<br/>REAL CHARTS.<br/><span className="text-muted-foreground">REAL JOURNEY.</span></h2><p className="mt-6 text-muted-foreground">A selection of shared trade examples, market setups and chart analysis.</p></div>
        <div className="mt-12 grid gap-4 md:grid-cols-12">{["MARKET STRUCTURE","LIQUIDITY MAP","PRICE ACTION","TRADE JOURNAL"].map((label,i)=><button key={label} onClick={()=>setLightbox(i)} className={`group reveal-item relative min-h-64 overflow-hidden border border-border bg-card text-left ${i===0||i===3 ? "md:col-span-7" : "md:col-span-5"}`}><ChartPlaceholder variant={i}/><span className="absolute bottom-5 left-5 text-xs font-semibold text-muted-foreground">{label}</span><span className="absolute right-5 top-5 flex size-9 items-center justify-center border border-border bg-background/70 opacity-0 backdrop-blur transition-opacity group-hover:opacity-100"><ExternalLink className="size-4" /></span></button>)}</div>
    ...
    <section id="reviews" className="section-pad"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end"><div className="reveal-item"><Eyebrow>TRUSTPILOT</Eyebrow><h2 className="mt-5 font-display text-4xl font-bold sm:text-6xl">WHAT THE<br/>COMMUNITY SAYS.</h2><div className="mt-8 flex items-end gap-4"><span className="font-display text-5xl font-bold">4.8 / 5</span><span className="mb-1 text-xs text-muted-foreground">46 Reviews</span></div><div className="mt-3 flex gap-1 text-primary" aria-label="5 stars"><Star className="fill-current"/><Star className="fill-current"/><Star className="fill-current"/><Star className="fill-current"/><Star className="fill-current"/></div><p className="mt-3 text-sm text-muted-foreground">46 Reviews on Trustpilot</p></div><div className="glass-panel reveal-item p-6 sm:p-10"><div className="flex items-center justify-between"><span className="text-xs font-bold text-primary">REVIEW GALLERY</span><div className="flex gap-2"><Button variant="outline" size="icon" onClick={()=>setReview((review+2)%3)} aria-label="Previous review"><ChevronLeft/></Button><Button variant="outline" size="icon" onClick={()=>setReview((review+1)%3)} aria-label="Next review"><ChevronRight/></Button></div></div><div className="mt-8 flex min-h-52 items-center justify-center border border-dashed border-border bg-background/40 p-8 text-center"><div><ShieldCheck className="mx-auto size-9 text-primary"/><p className="mt-4 font-display font-semibold">CLEANED REVIEW SCREENSHOT {review+1}</p><p className="mt-2 text-sm text-muted-foreground">Reserved for an authentic Trustpilot review image</p></div></div></div></div><Button variant="link" asChild className="mt-8 px-0"><a href={links.trustpilot} target="_blank" rel="noreferrer">VIEW ALL REVIEWS ON TRUSTPILOT <ArrowRight /></a></Button></div></section>

    <section className="section-pad border-y border-border bg-secondary/35"><div className="mx-auto max-w-7xl px-5 lg:px-8"><Eyebrow>COMMUNITY</Eyebrow><h2 className="mt-5 font-display text-4xl font-bold sm:text-6xl">FOLLOW THE JOURNEY.</h2><div className="mt-12 grid gap-px border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">{[[Youtube,"YOUTUBE","Free trading education and market content.",links.youtube],[Instagram,"INSTAGRAM","Follow the latest content and updates.",links.instagram],[Send,"TELEGRAM","Join the DhanOne3 trading community.",links.telegram],[MessageCircle,"DISCORD","Join the community and discussions.",links.discord]].map(([Icon,name,desc,url])=><a key={String(name)} href={String(url)} target="_blank" rel="noreferrer" className="group reveal-item bg-background p-7 transition-colors hover:bg-card"><Icon className="size-6 text-primary"/><h3 className="mt-14 font-display text-lg font-bold">{String(name)}</h3><p className="mt-3 min-h-12 text-sm leading-6 text-muted-foreground">{String(desc)}</p><ArrowRight className="mt-7 size-4 transition-transform group-hover:translate-x-1"/></a>)}</div></div></section>
    ...
    {lightbox !== null && <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/95 p-4 backdrop-blur" role="dialog" aria-modal="true" aria-label="Trade chart preview"><Button variant="outline" size="icon" className="absolute right-5 top-5" onClick={()=>setLightbox(null)}><X/></Button><div className="relative aspect-video w-full max-w-5xl overflow-hidden border border-border bg-card"><ChartPlaceholder variant={lightbox}/></div></div>}
  </main>;
}
...
function Field({label,error,children}:{label:string,error?:string,children:React.ReactNode}) { return <label className="mb-5 block"><span className="mb-2 block text-[10px] font-bold text-muted-foreground">{label}</span>{children}{error && <span className="mt-2 block text-xs text-destructive">{error}</span>}</label>; }
...
function MarketVisual() { return <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-[62%] lg:block"><svg className="size-full opacity-70" viewBox="0 0 900 700" fill="none" aria-hidden="true"><path d="M30 535 C135 500 155 540 250 455 S365 450 440 342 S580 350 640 250 S780 238 880 112" stroke="var(--primary)" strokeWidth="2" strokeDasharray="8 10" className="chart-dash"/><path d="M30 535 C135 500 155 540 250 455 S365 450 440 342 S580 350 640 250 S780 238 880 112 V700 H30Z" fill="url(#area)" opacity=".15"/><defs><linearGradient id="area" x1="450" y1="100" x2="450" y2="700"><stop stopColor="var(--primary)"/><stop offset="1" stopColor="var(--primary)" stopOpacity="0"/></linearGradient></defs>{[[100,430,60],[180,470,78],[290,370,55],[365,400,90],[475,300,76],[560,330,110],[670,220,65],[755,255,95]].map(([x,y,h],i)=><g key={i}><line x1={x} y1={y-h/2} x2={x} y2={y+h/2} stroke="var(--muted-foreground)"/><rect x={x-7} y={y-h/4} width="14" height={h/2} fill={i%2 ? "var(--muted-foreground)" : "var(--primary)"}/></g>)}</svg></div>; }
